...
`openxmlsdk-native` 是一个基于 .NET 10 Native AOT 技术的开源 C-ABI 包装库。它将微软官方 `DocumentFormat.OpenXml` 封装并导出为无外部运行时依赖的原生物理二进制文件（动态库与静态库），方便 Rust、C/C++、Go 及 Java/Kotlin (Android) 等语言进行跨平台 FFI 调用。
